...
def get_members_of_parliament(name="", parliament='DE'):
    query_string = """SELECT DISTINCT ?mdb WHERE {{
        ...
        LIMIT 16""".format(instance_of = INSTANCE_OF, human = HUMAN, position_held = POSITION_HELD, position_held_ps = cps(POSITION_HELD), memberParliamentDe=MEMBER_OF_PARLIAMENT[parliament], name = name)
    print(query_string)
    return query_string
